...
# Usage: ./move_to_closed.sh [--consolidate] <work_item_id> [<work_item_id> ...]
# Example: ./move_to_closed.sh TSK001 TSK002
# Example: ./move_to_closed.sh US001 FT001
# Example: ./move_to_closed.sh --consolidate TSK001 TSK002
...
if [ $# -eq 0 ]; then
    ...
    echo "Usage: ./move_to_closed.sh <work_item_id> [<work_item_id> ...]"
    echo "Example: ./move_to_closed.sh TSK001 TSK002"
    exit 1
fi
...
for item_id in "$@"; do
    ...
    # Find the matching file in the open directory
    found_files=()
    ...
    if [ ${#found_files[@]} -eq 0 ]; then
        echo "Warning: No file found for work item $item_id in open directory. Skipping..."
        continue
    ...
    for file in "${selected_files[@]}"; do
        ...
        target_file="$CLOSED_DIR/$filename"

        echo "Moving $filename to closed directory..."

        # Update the status in the file content
        sed -i 's/^Status:.*$/Status: Closed/' "$file"

        # Add completion date if it doesn't exist
        if ! grep -q "^Completion Date:" "$file"; then
            current_date=$(date +"%Y-%m-%d")
            # Find the line with "Status:" and append Completion Date after it
            sed -i "/^Status:/a Completion Date: $current_date" "$file"
        fi
        ...
        if grep -q "^Parent:" "$target_file"; then
            ...
            if [[ $parent_id =~ ^(EP|FT|US)[0-9]{3}$ ]]; then
                ...
                for parent_dir in "$OPEN_DIR" "$WORK_MGMT_DIR/backlog"; do
                    for parent_file in "$parent_dir"/${parent_id}*.md; do
                        if [ -f "$parent_file" ]; then
                            ...
                            if grep -q "$item_id" "$parent_file"; then
                                sed -i "s/$item_id - [^C].*/$item_id - Closed/" "$parent_file"
                                echo "Updated child status in parent file"
                            fi

                            # Check if all children are completed
                            if ! grep -q "- \(Open\|Backlog\)" "$parent_file"; then
                                echo "All child items are closed. Consider moving parent to closed as well."
                            fi
                        fi
                    done
                done
